...
export const httpProvider = axios.create({
    baseURL: process.env.NODE_ENV === 'development' ? 'http://dev.tulaoda.top:3000/mock/4/api' : 'http://dev.tulaoda.top:3000/mock/4/api',
    timeout: 60000,
})
    ...
    (err) => {
        if (err && err.response && err.response.status) {
            ...
            switch (status) {
                ...
                default:
                    typeof window !== 'undefined' && message.error((err.response && err.response.data && err.response.data.msg) || '未知错误!')
            }
        }
